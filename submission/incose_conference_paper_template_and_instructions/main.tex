...
We present a framework using typed simplicial complexes to formalize document quality assurance. Documents become vertices (0-simplices), verification and validation relationships become edges (1-simplices), and spec-guidance coupling edges (1-simplices) complete assurance triangles to form faces (2-simplices). Assurances require verification and validation of fitness-for-purpose with mandatory human accountability. LLM assistance is tracked but cannot substitute for human judgement and associated sign-off.

We demonstrate through self-reference: first specifications and guidance documents were drafted for this paper, then the content was iteratively generated with LLM assistance regulated by deterministic automated testing made possible by our framework. This paper is verified against its specification, validated against its guidance, and then provided with assurances which were subsequently audited. This proof-by-existence shows that the framework is operational; it demonstrates how engineering organizations may adopt AI assistance confidently while assuring quality, guaranteeing traceability and maintaining auditable chains of human responsibility.

\phantomsection
\subsubsection{Keywords}
verification and validation, quality assurance, topological methods, accountability, generative AI, human-in-the-loop
...
A second gap concerns accountability for validation. Verification can be automated: checking word counts, section presence, and format compliance requires no judgment. Validation is inherently subjective—assessing whether content is clear, rigorous, or practically useful requires human evaluation. This aligns with principles from cognitive systems engineering: automation should support rather than replace human expertise \parencite{Hollnagel2003, BoyEtAl2015}. When AI assists in generating content, who is responsible for judging its fitness? The answer cannot be ``the AI'' because language models cannot bear accountability. There must be a named human responsible for review, evaluation, and approval.
...
Section 2 reviews related work in verification and validation, requirements traceability, algebraic topology, test-driven development, and AI accountability. Section 3 presents the framework intuition based on optimization theory. Section 4 presents the framework architecture using a layered model consistent with the INCOSE SE Handbook. Section 5 demonstrates results through self-application and audit. Section 6 discusses implications and limitations. Section 7 concludes with key takeaways.
% ===== Background Section =====
\section{Background}
...
But TDD for documentation requires extension. Code tests verify behavior; they do not assess whether the code solves the right problem. Similarly, specification compliance verifies structure but not quality. The extension requires coupling: specifications (structural tests) paired with guidance (quality criteria).

\subsection{Assurance Cases and Safety Arguments}

Model-based assurance cases provide structured arguments for system safety and reliability. The OMG Structured Assurance Case Metamodel (SACM) defines standardized representations for assurance arguments \parencite{OMGSACM2023}, while Goal Structuring Notation (GSN) offers graphical syntax for safety cases \parencite{GSN}. These approaches decompose high-level claims into evidence through argumentation structures. Our framework complements this work: where assurance cases argue that a system meets safety requirements, our assurance triangles ensure that documents meet quality requirements through coupled verification and validation.
...
From a systems engineering perspective, AI agents are fundamentally not self-governing—every autonomous system is deployed \textit{by someone}, and there is always an accountable stakeholder \parencite{Zargham2024}. The term ``agent'' implies a principal who provisions, deploys and monitors that agent. Autonomy does not eliminate oversight; rather, it represents delegated control within clearly defined parameters. The principal defines the mission, establishes constraints, and retains responsibility for outcomes. LLMs are tools embedded within larger systems, not autonomous agents themselves; they generate text but remain the instruments of their operators and provisioners. The consequences of LLM use are realized within orchestrated systems where humans retain accountability.

Our framework provides oversight and accountability mechanisms through structural requirements: validation edges cannot exist without a human approver.
...
This section describes our system architecture across four layers: Conceptual, Functional, Logical and Physical. The conceptual architecture focuses on stakeholders and acceptable criteria regarding the delivered capabilities (not limited to technology delivery). The functional architecture describes what functions must be performed to deliver those capabilities. The logical architecture describes the mathematical structures, type system, processes and formal rules we've developed to enable the performance of these functions. The physical architecture describes the concrete implementation of those logics. 
...
\begin{enumerate}
\item \textbf{Requirements Traceability}: Auditable traces for document requirements through sequential (and nested) verification and validation cycles.
...
\textbf{Acceptance Criteria:} The framework is accepted when: (1) this paper is produced using the framework with complete assurance infrastructure; (2) paper is submitted to INCOSE IS 2026; (3) named human attests to all assured documents; (4) assurance audit shows 100\% vertex document coverage; (5) user continues to use the framework to produce requirements intensive documents after INCOSE IS paper pilot.
...
\centering
\small
\begin{tabular}{p{0.145\linewidth}p{0.19\linewidth}p{0.14\linewidth}p{0.36\linewidth}}
\toprule
...
\midrule
Audit Tool & Python script & Coverage and topological analysis \\
\midrule
...
\begin{enumerate}
    ...
    \item  Assurance face \(a\) asserts a human signed validation edge \((d,g)\) from document vertex $d$  to guidance vertex $g$.
...
As a consequence of the pairings of documents to assurances (d, $a_d$), and the presence of the root vertex, a valid assurance complex satisfies $V - F = 1$ when each document is assured exactly once.  It is possible to assure the same document $d$ against more than one coupling edge $(s,g)$, resulting in valid assurance complexes with \(V - F<1\). The invariant \(V-F\le 1\) is necessary but not sufficient for an assurance complex to be valid, and therefore can be used to spot check and quickly identify invalid assurance complexes.
...
\textbf{Phase 3: Content Assembly.} This paper was drafted iteratively, with each section verified for structural compliance and validated for fitness-for-purpose. Verification was automated (type \& schema checks); validation required human judgment against quality criteria. Both verification and validation edges were created for each section, with coupling edges ensuring spec-guidance alignment. This process was inherently loopy but automation allowed rapid propagation of changes and facilitated refreshing human approvals after changes were merged.

\textbf{Phase 4: Serialization.} The assured Markdown source was transformed to LaTeX via a modified version of Phase 3 and this PDF was compiled for submission. Acknowledged gap: the verified sources (Markdown and LaTeX files) differ from the submitted artifact (PDF). The source repository remains the authoritative record.
...
\node[input]  (approver) at (-3,-6) {approver};
\draw[arrow] (approver) -- node[left, font=\tiny] {approval} (validate);
...
This section presents results from auditing the assurance complex output at the end of Phase 3. Each non-root vertex must have at least one assurance face.

\subsubsection{Audit Algorithm}
Coverage is complete when all upstream dependencies trace to the boundary complex without encountering unassured vertices.
\columnbreak
\scriptsize{
\begin{verbatim}
...
This sequence demonstrates the framework operating: a gap was detected through systematic auditing, root cause was identified, fix was implemented, and verification confirmed resolution. The meta-observation is that the framework's tooling caught a tooling defect—its self-referential nature strengthened rather than undermined confidence. 

Early experiments demonstrated that allowing forward propagation of document updates by generative AI results in edit cascades. This proved to be as dangerous as it was powerful. The development and integration of automated regression testing was crucial for effectively regulating this behavior.
...
\begin{enumerate}
    ...
    \item A human author validated this paper's fitness-for-purpose
    \item All dependency documents (supporting documents, specs, and guidance) have been assured with accountable humans signing the associated validations.
\end{enumerate}
\end{enumerate}

The traceability and accountability properties are structural, not procedural. The assurance complex checks for compliance with the framework axioms via \texttt{audit\_assurance\_chart.py}. The \texttt{human\_approver} field is required by the type system—not by policy, but by data structure. Github Actions automate schema validation and matching the signer to the Github Username making the git commit; this prevents validation edges without named signers from being committed to the repository.
...
The self-referential foundation addresses a fundamental challenge: how to specify the specification language itself. The boundary complex resolves this through an axiomatic root vertex that grounds the type system while maintaining topological validity. This pattern—introducing a distinguished element to transform degenerate structures into well-formed complexes—appears throughout mathematics and suggests broader applicability. The use of formal mathematical boundary conditions is well established in engineering practice \parencite{SzaboBabuska2011}. Our approach invites an interpretation of document relationships that is reminiscent of finite element analysis.

\textbf{Current limitations} (1) the framework requires computational infrastructure (Python scripts, version control, continuous integration) which may not be accessible to all organizations; (2) the type system is currently domain-specific to technical documentation and would require adaptation for other document types; (3) human accountability enforcement depends on organizational commitment to maintaining validation discipline; (4) the boundary complex pattern has been demonstrated only for the foundational four-vertex case and scaling properties remain to be characterized; (5) stabilizing and packaging software components, as well as building document-type registries are needed to improve user experience and to mitigate user-error related risks.

\textbf{Future work} (1) compositional patterns for assembling larger complexes from assured sub-complexes, potentially leveraging categorical frameworks \parencite{Spivak2014}; (2) integration with existing requirements management tools; (3) formal verification of the type system's soundness using techniques from type theory \parencite{Pierce2002} or Hoare logic \parencite{Hoare1969}; and (4) application to domains beyond technical documentation such as regulatory compliance, academic peer review, or governance documentation. 

Submitting documents to git-based version control with verifiable properties (including the presence of signatures for accountable humans) opens the door to applying Model-Based Systems Engineering to the aspects of organizational analysis, design, \textit{implementation} and \textit{quality assurance} which have heretofore been handled by bureaucracies \parencite{ZarghamBenMeir2026}. %The authors' focus is on engineering organizations; our application of this framework introduce actors, activities and resources into our topological modeling framework. Typed edges represent relationships, and faces are used to codify constraints which must be satisfied; research, development and operationalization will proceed along this direction.

\section{Conclusions}

This paper introduced a topological framework for document assurance that structurally enforces verification, validation, and human accountability requirements. The framework represents documents as vertices, quality relationships as typed edges, and assurances as 2-simplices in a typed simplicial complex, with human accountability guaranteed by type system constraints on validation edges.
...
\textbf{Human Accountability:} The named human approver (author) reviewed all AI-generated content and takes full responsibility for the paper's claims and conclusions. All validation edges in the supporting infrastructure bear the \texttt{human\_approver: <author>} attribution, enforced by type system constraints and continuous integration checks.

This disclosure follows the methodological precedent established in Ghrist's ``The Forge'' \parencite{Ghrist2025Forge}, which documents a comparable process of AI-assisted authorship in which the human provides intellectual substance, direction, oversight, and approval while AI contributes drafting capability. The use of LLM assistance in document preparation exemplifies this paper's central thesis: computational tools can accelerate generation and verification tasks while preserving human accountability through structural enforcement of validation requirements.
