...
We demonstrate through self-reference: first specifications and guidance documents were drafted for this paper, then the content was iteratively generated with LLM assistance regulated by deterministic automated testing made possible by our framework. This paper is verified against its specification, validated against its guidance, and the provided with assurances which were subsequently audited. This proof-by-existence shows the framework is operational, demonstrating how engineering organizations may adopt AI assistance confidently while assuring quality, traceability and maintaining auditable chains of human responsibility.
...
As a consequence of the pairings of documents to assurances (d, $a_d$), and the presence of the root vertex, a valid assurance complex satisfies $V - F = 1$ when each document is assured exactly once.  It is possible to assure the same document $d$ against more than one coupled $(s,g)$, resulting in valid assurance complexes with \(V - F<1\). The invariant \(V-F\le 1\) is necessary but not sufficient for an assurance complex to be valid, and therefore can be used to spot check and quickly identify invalid assurance complexes.
...
Early experiments demonstrated that allowing forward propagation of document updates by generative AI results edit cascades. This proved to be as dangerous as it was powerful. The development and integration of automated regression testing proved to be crucial for effectively regulating this behavior.
...
The traceability and accountability properties are structural, not procedural. The assurance complex checks for compliance with the framework axioms via \texttt{audit\_assurance\_chart.py}. The \texttt{human\_approver} field is required by the type system—not by policy, but by data structure. \textit{Github Actions} automate schema validation and matching the signer to the \textit{Github Username} making the git commit; this prevents validation edges without named signers from being committed to the repository.
