...
def main():
    ...
    for type_name, spec in parser.templates.items():
        print(f"\n{type_name}:")
        print(f"  Extends: {spec.extends}")
        print(f"  Frontmatter requirements: {len(spec.frontmatter_requirements)}")
        print(f"  Body requirements: {len(spec.body_requirements)}")
        print(f"  Tag requirements: {len(spec.tag_requirements)}")

        if '--verbose' in sys.argv:
            for req in spec.frontmatter_requirements:
                cond_str = f" (if {req.conditional})" if req.conditional else ""
                value_str = f" = {req.expected_value}" if req.expected_value else ""
                print(f"    - {req.field}{value_str}{cond_str}")
            for section in spec.body_requirements:
                print(f"    - Body: {section}")
            for tag in spec.tag_requirements:
                print(f"    - Tag: {tag}")
